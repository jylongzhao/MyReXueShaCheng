...
class Monomer:public CCSprite,public FigureDelegate
{
protected:
...
public:
	...
	CCPoint getHandPointRelativeFootOffset();

	CCPoint getHandPoint();

	CCPoint getHurtPointRelativeFootOffset();

	CCPoint getHurtPoint();

	Monomer* getAgainstMeOfFirst();

	CCRect getRect();

	int getAlphaWithPoint(CCPoint point);
...
protected:
	...
	void playEffect(CCNode* node, const char* path);
...
//	void pushFlutteringFairy(FlutteringFairy* fairy);
	...
	CCAction* delayCallBack(float time, SEL_CallFunc callFunc);
...
#endif /* defined(__mir_mobile__Monomer__) */
