...
bool GameScene::init()
{
	...
	CCSpriteFrameCache::sharedSpriteFrameCache()->addSpriteFramesWithFile("texture_set/blood_return.plist");

	m_selected = CCSprite::create("ui/tray_self.png");
	m_selected->retain();
...
void GameScene::replaceBgMap(int mapID, const int born)
{
	...
	//m_touchMouse->setPosition(CCPoint(-100, -100));
...
void GameScene::touchProtected()
{
	do
	{
		...
		CCCallFunc* callFunc = CCCallFunc::create(this, callfunc_selector(GameScene::untouchProtected));
...
void GameScene::updateImageDisplay(cocos2d::CCRect &showRect, cocos2d::CCRect &hideRect)
{
...
//
//vector<Monomer*> GameScene::getMonmerVecIsLenght(CCPoint point, int lenght)
//{
//	vector<Monomer*> vec;
//	CCArray* enemyKeys = m_enemyDictionary->allKeys();
//	if (enemyKeys)
...
MapPoint GameScene::getPlayerDirection()
{
	...
	CCPoint begin = Player::sharePlayer()->getPosition();
	CCPoint end = m_bgMap->convertToNodeSpace(m_playerDirection);
...
{
	//if (CCDirector::sharedDirector()->getRunningScene()->getChildByTag(0xffffff))
	//	return;
