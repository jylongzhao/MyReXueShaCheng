...
static CCSize GRID_SIZE = CCSize(60,32);
MapPoint::MapPoint():x(0),z(0){
...
MapPoint::MapPoint(const CCPoint& point)
...
CCPoint MapPoint::getCCPointValue() const
{
	CCPoint point = CCPoint(x * GRID_SIZE.width, z * GRID_SIZE.height);
	point = ccpAdd(point, GRID_SIZE/2);
	return point;
}

CCSize MapPoint::getCCSizeValue() const
{
	CCSize size = CCSize(x * GRID_SIZE.width, z * GRID_SIZE.height);
	return size;
}
...
void MapPoint::setGridSize(cocos2d::CCSize size)
{
	GRID_SIZE = size;
}